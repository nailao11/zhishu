...
列表里可以随时「禁用/启用」某个词：禁用即暂停定时抓取，历史数据保留，随时可恢复。
...
装的时候自动加好 cron，每天 15:05 触发（数据源一般午后才更新前一天的数据）。
为了不像机器，脚本先随机延迟至多 20 分钟才开始，抓取时 5 个词一组、组间随机停顿 20~50 秒，
80 个词约 10 分钟跑完。改时间：
...
关键词添加满 60 天自动删除、不再抓取（`ZHISHU_KEYWORD_TTL_DAYS` 可改，0 表示不自动删）。
...
PATCH /api/keywords/{keyword}       启用/禁用 {"enabled":true}
...
POST  /api/diagnose                 网络诊断（测出口 IP / 代理 / 凭证链路）
GET   /api/runs                     任务运行记录
```
